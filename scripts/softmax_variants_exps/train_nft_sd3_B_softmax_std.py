...
import numpy as np
from weight_mappings import compute_variant_weights, validate_weight_settings
...
# Experiment B: softmax_std; formulas and sweep are in assets/softmax_variant/variants.tex.
WEIGHT_VARIANT = "B"
flags.DEFINE_float("weight_c", 0.5, "Near-max fraction or mapping temperature c (A/B/D/F).")
flags.DEFINE_float("weight_p", 1.0, "Positive-part power exponent p (E).")
...
# A--F use compute_variant_weights on raw rewards; the original helper is retained above.
...
def main(_):
    config = FLAGS.config
    validate_weight_settings(WEIGHT_VARIANT, FLAGS.weight_c, FLAGS.weight_p)
    weight_settings = {
        "variant": WEIGHT_VARIANT,
        "c": FLAGS.weight_c,
        "p": FLAGS.weight_p,
        "group_size": config.sample.num_image_per_prompt,
        "reward_input": "raw reward_fn output; no advantage normalization or clipping",
    }
    ...
    valid_trajectory_alpha_predictions = {"old_prediction"}  # Paper: denominator uses x_old.
    ...
    logger.info(f"\n{config}")
    logger.info("Weight mapping: %s", weight_settings)
    if is_main_process(rank):
        writer.add_text("weight_mapping", json.dumps(weight_settings, indent=2), 0)
    ...
    for epoch in range(first_epoch, config.num_epochs):
        ...
        for i in tqdm(
            ...
            transformer_ddp.module.set_adapter("default")  # Paper: roll out with current theta.
        ...
        if config.per_prompt_stat_tracking:
            ...
            # advantages = stat_tracker.update(prompts_all_decoded, gathered_rewards_dict["avg"])
            stat_tracker.update(prompts_all_decoded, gathered_rewards_dict["avg"])  # Reward logging only.
            ...
            # avg_rewards_all = gathered_rewards_dict["avg"]
            # advantages = (avg_rewards_all - avg_rewards_all.mean()) / (avg_rewards_all.std() + 1e-4)
            pass  # Weights always come from raw prompt-group rewards below.
        ...
        # )
        importance_weights = compute_variant_weights(
            prompts_all_decoded,
            rollout_batch_ids_all,
        ...
        normalized_advantages = importance_weights - 1.0
        # Keep the base buffer interface; this signal is NOT clipped or standardized.
        advantages = np.repeat(normalized_advantages[:, None], num_train_timesteps, axis=1)
        # Every group's mean(w) is 1, hence 1 + beta * mean(w - 1) = 1.
        prompt_normalizers = np.ones_like(importance_weights)
        if is_main_process(rank):
            log_scalars(writer, {
                "weights/mean": importance_weights.mean(),
                "weights/min": importance_weights.min(),
                "weights/max": importance_weights.max(),
                "weights/std": importance_weights.std(),
                "weights/zero_fraction": np.mean(importance_weights == 0.0),
            }, global_step)
        ...
        for inner_epoch in range(config.train.num_inner_epochs):
            ...
            for i, train_sample_batch in tqdm(
                ...
                # Paper: fresh t ~ U(0,1) for each image and training pass.
                # Retain the base schedule buffer, but replace its values before
                # the shared t/1000 interpolation and all three model forwards.
                train_sample_batch["timesteps"] = 1000.0 * torch.rand_like(
                    train_sample_batch["timesteps"], dtype=torch.float32
                )
