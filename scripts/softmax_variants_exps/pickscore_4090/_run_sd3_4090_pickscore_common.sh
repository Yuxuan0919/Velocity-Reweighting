...
WEIGHT_VARIANT="${WEIGHT_VARIANT:-A}"
WEIGHT_C="${WEIGHT_C:-0.0}"
WEIGHT_P="${WEIGHT_P:-1.0}"
EXPERIMENT_TAG="${EXPERIMENT_TAG:-${WEIGHT_VARIANT}-c${WEIGHT_C}-p${WEIGHT_P}}"
EXPERIMENT_BETA="${EXPERIMENT_BETA:-1.0}"

case "${WEIGHT_VARIANT}" in
  A) TRAIN_SCRIPT="scripts/softmax_variants_exps/train_nft_sd3_A_near_max.py" ;;
  B) TRAIN_SCRIPT="scripts/softmax_variants_exps/train_nft_sd3_B_softmax_std.py" ;;
  C) TRAIN_SCRIPT="scripts/softmax_variants_exps/train_nft_sd3_C_linear.py" ;;
  D) TRAIN_SCRIPT="scripts/softmax_variants_exps/train_nft_sd3_D_sigmoid.py" ;;
  E) TRAIN_SCRIPT="scripts/softmax_variants_exps/train_nft_sd3_E_power.py" ;;
  F) TRAIN_SCRIPT="scripts/softmax_variants_exps/train_nft_sd3_F_softmax_l1.py" ;;
  *) echo "WEIGHT_VARIANT must be A, B, C, D, E or F" >&2; exit 2 ;;
esac

# LOGDIR="${LOGDIR:-${REPO_DIR}/logs/weight_exps/weight_exps_pickscore}"
LOGDIR="${LOGDIR:-${REPO_DIR}/logs/softmax_variants_exps/pickscore}"
...
# SAVE_DIR="${SAVE_DIR:-${REPO_DIR}/outputs/weight_exps/weight_exps_pickscore/sd35_pickscore_4090_${WORLD_SIZE}gpu_nft_ours-4selection-soft-${EXPERIMENT_TAG}-KL1e-4-beta${EXPERIMENT_BETA}-fulltime}"
SAVE_DIR="${SAVE_DIR:-${REPO_DIR}/outputs/softmax_variants_exps/pickscore/sd35_pickscore_4090_${WORLD_SIZE}gpu_nft_softmax-variants-${EXPERIMENT_TAG}-KL1e-4-beta${EXPERIMENT_BETA}-fulltime}"
# RUN_NAME="${RUN_NAME:-sd35_pickscore_4090_${WORLD_SIZE}gpu_nft_ours-4selection-soft-${EXPERIMENT_TAG}-KL1e-4-beta${EXPERIMENT_BETA}-fulltime}"
RUN_NAME="${RUN_NAME:-sd35_pickscore_4090_${WORLD_SIZE}gpu_nft_softmax-variants-${EXPERIMENT_TAG}-KL1e-4-beta${EXPERIMENT_BETA}-fulltime}"

# mkdir -p "${LOGDIR}" "${SAVE_DIR}" "${REPO_DIR}/.cache"

# source "${CONDA_ROOT}/bin/activate" "${CONDA_ENV}"
# DRY_RUN=1 checks and prints the command without activating the environment or using GPUs.
if [[ "${DRY_RUN:-0}" != "1" ]]; then
  mkdir -p "${LOGDIR}" "${SAVE_DIR}" "${REPO_DIR}/.cache"
  source "${CONDA_ROOT}/bin/activate" "${CONDA_ENV}"
fi
...
# echo "Launching PickScore ${EXPERIMENT_TAG} on node ${NODE_RANK}/${NNODES}: ${NNODES}x${NPROC_PER_NODE}=${WORLD_SIZE} GPUs, per-device batch=${PER_DEVICE_BATCH}, accumulation=${GRADIENT_ACCUMULATION_STEPS}, effective batch=${EFFECTIVE_BATCH}, scheme=${MASS_SHIFT_SCHEME}, rho=${MASS_SHIFT_RHO}, beta=${EXPERIMENT_BETA}"
echo "Launching PickScore ${EXPERIMENT_TAG} on node ${NODE_RANK}/${NNODES}: ${NNODES}x${NPROC_PER_NODE}=${WORLD_SIZE} GPUs, per-device batch=${PER_DEVICE_BATCH}, accumulation=${GRADIENT_ACCUMULATION_STEPS}, effective batch=${EFFECTIVE_BATCH}, variant=${WEIGHT_VARIANT}, c=${WEIGHT_C}, p=${WEIGHT_P}, beta=${EXPERIMENT_BETA}"
...
# torchrun "${TORCHRUN_DISTRIBUTED_ARGS[@]}" --nproc_per_node="${NPROC_PER_NODE}" "${TRAIN_SCRIPT}" \
  ...
  # "$@"
TRAIN_COMMAND=(torchrun "${TORCHRUN_DISTRIBUTED_ARGS[@]}" --nproc_per_node="${NPROC_PER_NODE}" "${TRAIN_SCRIPT}" \
  --config=config/nft.py:sd3_pickscore \
  --config.pretrained.model="${SD3_MODEL}" \
  --config.logdir="${LOGDIR}" \
  --config.save_dir="${SAVE_DIR}" \
