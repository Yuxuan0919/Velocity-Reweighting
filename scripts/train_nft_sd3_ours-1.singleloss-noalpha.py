        ...
        for inner_epoch in range(config.train.num_inner_epochs):
            ...
            for i, train_sample_batch in tqdm(
                ...
                for j_idx, j_timestep_orig_idx in tqdm(
                    ...
                    with torch.no_grad():
                        fake_weight_factor = t_expanded.square().clip(min=1e-8)
                    positive_loss = ((x0_prediction - x0) ** 2 / fake_weight_factor).mean(
                    ...
                    with torch.no_grad():
                        fake_negative_weight_factor = t_expanded.square().clip(min=1e-8)
                    negative_loss = ((negative_x0_prediction - x0) ** 2 / fake_negative_weight_factor).mean(
                        dim=tuple(range(1, x0.ndim))
                    )
                    
                    # log weight factor
                    loss_terms["positive_weight_factor"] = weight_factor.detach()
                    loss_terms["negative_weight_factor"] = negative_weight_factor.detach()
