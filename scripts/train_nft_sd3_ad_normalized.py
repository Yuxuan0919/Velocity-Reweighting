        ...
        for inner_epoch in range(config.train.num_inner_epochs):
            ...
            for i, train_sample_batch in tqdm(
                ...
                for j_idx, j_timestep_orig_idx in tqdm(
                    ...
                    # Normalize advantages into soft add/delete scores, then build the
                    # batch-level mass-normalized target from normalized_target.tex.
                    ...
                    batch_signed_mass_sum = signed_edit_mass.detach().sum()
                    batch_signed_mass_count = torch.tensor(float(signed_edit_mass.numel()), device=device)
                    mass_stats = torch.stack([batch_signed_mass_sum, batch_signed_mass_count])
                    if world_size > 1:
                        dist.all_reduce(mass_stats, op=dist.ReduceOp.SUM)
                    batch_signed_mass_mean = mass_stats[0] / mass_stats[1].clamp(min=1.0)
                    batch_total_mass = 1.0 + batch_signed_mass_mean
                    if batch_total_mass.item() <= 0:
                        raise ValueError(
                            f"Mass-normalized target requires positive total mass, got M_B={batch_total_mass.item():.6f}"
                        )
                    normalized_edit_coeff = signed_edit_mass / batch_total_mass
                    normalized_edit_coeff_expanded = normalized_edit_coeff.view(-1, *([1] * (x0.ndim - 1)))

                    # x-space counterpart of the mass-normalized target:
                    # x_bar = x_old + (w_i / M_B) * (x0 - x_old),
                    # where M_B = 1 + mean_i[w_i] for the current distributed micro-batch.
                    x0_prediction = xt - t_expanded * forward_prediction
                    x0_old_prediction = xt - t_expanded * old_prediction.detach()
                    x0_add_delete_target = (
                        x0_old_prediction + normalized_edit_coeff_expanded * (x0 - x0_old_prediction)
                    )
                    with torch.no_grad():
                        weight_factor = (
                    ...
                    ori_policy_loss = policy_loss
                    policy_loss = (ori_policy_loss * config.train.adv_clip_max).mean()
                    loss_terms["signed_edit_mass_mean"] = signed_edit_mass.mean().detach()
                    loss_terms["batch_signed_mass_mean"] = batch_signed_mass_mean.detach()
                    loss_terms["batch_total_mass"] = batch_total_mass.detach()
                    loss_terms["normalized_edit_coeff_mean"] = normalized_edit_coeff.mean().detach()
                    loss_terms["normalized_edit_coeff_abs_mean"] = normalized_edit_coeff.abs().mean().detach()
