...
def main(_):
    ...
    # Keep the training timestep candidate schedule separate from the rollout/eval
    # schedules that pipeline_with_logprob installs on pipeline.scheduler.
    training_scheduler = FlowMatchEulerDiscreteScheduler.from_config(pipeline.scheduler.config)
    timestep_sampling_num_steps = int(config.train.timestep_sampling_num_steps)
    max_timestep_sampling_num_steps = int(training_scheduler.config.num_train_timesteps)
    if not 1 <= timestep_sampling_num_steps <= max_timestep_sampling_num_steps:
        raise ValueError(
            "config.train.timestep_sampling_num_steps must be in "
            f"[1, {max_timestep_sampling_num_steps}], got {timestep_sampling_num_steps}."
        )

    # Preserve the original full 1000-point training schedule by default. For
    # smaller values (for example, eval_num_steps=40), construct the same
    # inference grid that FlowMatchEulerDiscreteScheduler uses for evaluation.
    if timestep_sampling_num_steps < max_timestep_sampling_num_steps:
        training_scheduler.set_timesteps(timestep_sampling_num_steps, device=device)

    training_schedule_timesteps = training_scheduler.timesteps.to(device)
    num_timestep_candidates = len(training_schedule_timesteps)
    training_schedule_sigmas = training_scheduler.sigmas[:num_timestep_candidates].to(device)
    assert num_timestep_candidates == timestep_sampling_num_steps
    assert len(training_schedule_sigmas) == num_timestep_candidates
    logger.info(
        "Uniform training timestep sampling will use %d candidate schedule positions.",
        num_timestep_candidates,
    )
    ...
    for epoch in range(first_epoch, config.num_epochs):
        ...
        total_batch_size_filtered = filtered_samples["latents_clean"].shape[0]
        ...
        for inner_epoch in range(config.train.num_inner_epochs):
            ...
            for i, train_sample_batch in tqdm(
                ...
                # Draw the configured number of IID indices uniformly from the
                # configured shifted FlowMatch candidate schedule. Since the
                # schedule is stored in descending order, every candidate position
                # from high to low noise has equal probability.
                timestep_indices = torch.randint(
                    low=0,
                    high=num_timestep_candidates,
                    size=(current_micro_batch_size, num_train_timesteps),
                    device=device,
                )
                sampled_train_timesteps = training_schedule_timesteps[timestep_indices]
                sampled_train_sigmas = training_schedule_sigmas[timestep_indices]

                # Loop over the independently sampled training timesteps.
                for j_idx, j_timestep_orig_idx in tqdm(
                    ...
                    model_timestep = sampled_train_timesteps[:, j_idx]
                    t = sampled_train_sigmas[:, j_idx]
                    ...
                    with torch_autocast(enabled=enable_amp, dtype=mixed_precision_dtype):
                        ...
                        with torch.no_grad():  # Reference model part
                            ...
                            if config.use_lora:
                                ...
                                assert False
                    loss_terms = {}
                    loss_terms["train_timestep"] = model_timestep.mean().detach()
                    loss_terms["train_sigma"] = t.mean().detach()
