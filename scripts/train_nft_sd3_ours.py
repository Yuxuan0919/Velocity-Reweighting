...
# def compute_reinforced_flow_weights(prompts, rollout_batch_ids, rewards, advantage_clip, coverage_beta, epsilon):
#     r"""Compute per-rollout \hat A, W, and prompt-wise \bar Z from Algorithm 1."""
#     prompts = np.asarray(prompts)
#     rollout_batch_ids = np.asarray(rollout_batch_ids)
#     rewards = np.asarray(rewards, dtype=np.float64)
#     if rewards.ndim > 1:
#         # The same trajectory reward is repeated over training timesteps by the
#         # existing data pipeline; Algorithm 1 defines one weight per clean sample.
#         rewards = rewards[:, 0]
#     if advantage_clip <= 0:
...
#         raise ValueError("prompts, rollout_batch_ids, and rewards must have the same length")
#
#     normalized_advantages = np.empty_like(rewards)
#     importance_weights = np.empty_like(rewards)
#     prompt_normalizers = np.empty_like(rewards)
#
#     # A prompt may be drawn again in a later sampling batch.  Treat each draw as
#     # a separate c_j so that its K rollouts use exactly their own reward group.
...
#         prompt_rewards = rewards[sample_indices]
#         centered_rewards = prompt_rewards - prompt_rewards.mean()
#         reward_variance = np.mean(centered_rewards**2)
#         prompt_advantages = np.clip(
#             centered_rewards / np.sqrt(reward_variance + epsilon),
#             -advantage_clip,
#             advantage_clip,
#         ) / advantage_clip
#         prompt_weights = np.maximum(epsilon, 1.0 + prompt_advantages)
#         prompt_z_bar = 1.0 + coverage_beta * np.mean(prompt_weights - 1.0)
#
#         normalized_advantages[sample_indices] = prompt_advantages
#         importance_weights[sample_indices] = prompt_weights
