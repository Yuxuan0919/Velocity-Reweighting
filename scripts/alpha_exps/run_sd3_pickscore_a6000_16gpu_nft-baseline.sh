...
TRAIN_SCRIPT="scripts/alpha_exps/train_nft_sd3_origin.py"
...
RUN_NAME="${RUN_NAME:-sd35_pickscore_nft_baseline-KL1e-4-beta1.0-fulltime_r${RUN_INDEX}_${GPU_TYPE}}"
...
echo "Launching PickScore baseline on node ${NODE_RANK}/${NNODES}: ${NNODES}x${NPROC_PER_NODE}=${WORLD_SIZE} GPUs, per-device batch=${PER_DEVICE_BATCH}, accumulation=${GRADIENT_ACCUMULATION_STEPS}, effective batch=${EFFECTIVE_BATCH}"
