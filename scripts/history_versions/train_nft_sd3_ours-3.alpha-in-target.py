                ...
                for j_idx, j_timestep_orig_idx in tqdm(
                    ...
                    with torch.no_grad():
                        ...
                        trajectory_alpha_base_prediction = (
                            old_prediction
                            if trajectory_alpha_prediction == "old_prediction"
                            else forward_prediction
                        )
                        ...
                        trajectory_alpha_normalized = trajectory_alpha / trajectory_alpha.mean()
                        correction_coefficient = (
                            float(config.beta) * (importance_weight - 1)
                        )
                        ...
                        target_velocity = old_prediction.detach().float() + correction_coefficient_expanded * (
                            trajectory_alpha_normalized * velocity_discrepancy
                        )

                    target_velocity_loss = (trajectory_alpha * (forward_prediction - target_velocity) ** 2).mean(
