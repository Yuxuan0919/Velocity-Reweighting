...
def compute_reinforced_flow_weights(
    ...
    # Keep the original Delta for samples whose selection result is weaker.
    # This makes selection boost-only; the original retained-mass budget is no
    # longer an equality after the restored signals are included.
    attenuated = np.abs(selected_advantages) < np.abs(normalized_advantages)
    selected_advantages[attenuated] = normalized_advantages[attenuated]
