...
def _group_quality_adaptive_quantile(group_rewards, quantile_min, quantile_max):
    """Compute one prompt group's target quantile from its own mean reward."""
    group_mean_reward = float(np.mean(group_rewards))
    # Do not special-case the sign or range of the group mean. The configured
    # quantile bounds are the only constraint on the resulting target quantile.
    group_quantile_level = float(
        np.clip(1.0 - group_mean_reward, quantile_min, quantile_max)
    )
    return group_mean_reward, group_quantile_level
...
def compute_quality_adaptive_quantile_weights(
    ...
    r"""Compute per-prompt quality-adaptive quantile advantages and mass weights."""
    prompts = np.asarray(prompts)
    rollout_batch_ids = np.asarray(rollout_batch_ids)
    ...
    # Keep the global reward mean for monitoring only. It no longer determines
    # the target quantile used by every prompt group.
    policy_quality = float(np.mean(normalized_rewards))
    # Original global quality-adaptive quantile retained for comparison:
    # quantile_level = float(np.clip(1.0 - policy_quality, quantile_min, quantile_max))
    # Original unbounded global adaptive quantile retained for comparison:
    ...
    attainable_target_quantiles = []
    group_mean_rewards = []
    group_quantile_levels = []
    degenerate_group_count = 0
    for sample_indices in prompt_groups.values():
        sample_indices = np.asarray(sample_indices, dtype=np.int64)
        group_rewards = normalized_rewards[sample_indices]
        group_mean_reward, group_quantile_level = _group_quality_adaptive_quantile(
            group_rewards, quantile_min, quantile_max
        )
        group_mean_rewards.append(group_mean_reward)
        group_quantile_levels.append(group_quantile_level)
        boundary, attainable_fraction = _nearest_attainable_quantile_boundary(
            group_rewards, group_quantile_level
        )
        ...
        if attainable_fraction is None:
            ...
            attainable_target_quantiles.append(group_quantile_level)
    ...
    attainable_target_quantiles = np.asarray(attainable_target_quantiles, dtype=np.float64)
    group_mean_rewards = np.asarray(group_mean_rewards, dtype=np.float64)
    group_quantile_levels = np.asarray(group_quantile_levels, dtype=np.float64)
    if len(attainable_fractions) > 0:
        attainable_errors = np.abs(attainable_fractions - attainable_target_quantiles)
    ...
    metrics = {
        ...
        # Backward-compatible scalar aggregate. The actual quantile is computed
        # independently for every group and is summarized by the fields below.
        "quantile_level": float(np.mean(group_quantile_levels)),
        "group_mean_reward_mean": float(np.mean(group_mean_rewards)),
        "group_mean_reward_min": float(np.min(group_mean_rewards)),
        "group_mean_reward_max": float(np.max(group_mean_rewards)),
        "group_quantile_mean": float(np.mean(group_quantile_levels)),
        "group_quantile_min": float(np.min(group_quantile_levels)),
        "group_quantile_max": float(np.max(group_quantile_levels)),
...
def log_first_prompt_reward_distributions(
    ...
    for prompt_index, (rollout_batch_id, prompt) in enumerate(selected_groups):
        ...
        group_mean_reward, group_quantile_level = _group_quality_adaptive_quantile(
            group_normalized_rewards, quantile_min, quantile_max
        )
        # Original global target-quantile use retained for comparison:
        # baseline_reward, attainable_quantile = _nearest_attainable_quantile_boundary(
        #     group_normalized_rewards, quantile_level
        # )
        baseline_reward, attainable_quantile = _nearest_attainable_quantile_boundary(
            group_normalized_rewards, group_quantile_level
        )
        ...
        logger.info(
            "Epoch %d prompt_%d (rollout batch %d, %d samples) group mean reward: %.6f; "
            "target quantile: %.6f; "
        ...
        writer.add_scalar(f"{tag}/target_quantile", group_quantile_level, step)
            ...
            + json.dumps(
                {
                    ...
                    "target_quantile": group_quantile_level,
...
def main(_):
    ...
    for epoch in range(first_epoch, config.num_epochs):
        ...
        if is_main_process(rank):
            # Original global-quantile logging call retained for comparison:
            # log_first_prompt_reward_distributions(
            #     writer,
            #     prompts_all_decoded,
            #     rollout_batch_ids_all,
        ...
        if is_main_process(rank):
            ...
            # Original global-quality/global-quantile log retained for comparison:
            # logger.info(
            ...
            # )
            logger.info(
                "Global reward mean: %.6f; group quantile mean/min/max: %.6f/%.6f/%.6f; "
                "reward std: %.6f; positive/negative mass ratio: %.6f/%.6f",
                quality_adaptive_metrics["policy_quality"],
                quality_adaptive_metrics["group_quantile_mean"],
                quality_adaptive_metrics["group_quantile_min"],
                quality_adaptive_metrics["group_quantile_max"],
                quality_adaptive_metrics["global_reward_std"],
                quality_adaptive_metrics["positive_mass_ratio"],
                quality_adaptive_metrics["negative_mass_ratio"],
            )
