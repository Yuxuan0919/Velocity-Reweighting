...
ASYMMETRIC_MASS_SHIFT_SCALE=1.4142135623730951
ASYMMETRIC_SCALE_TAG=c1p4142
SAVE_DIR="${SAVE_DIR:-${REPO_DIR}/outputs/nft_sd3_geneval_h200_8gpu_nft_ours-asymmetric-${ASYMMETRIC_SCALE_TAG}-KL1e-4-beta1.0-fixshift}"
RUN_NAME="${RUN_NAME:-sd35_geneval_h200_8gpu_nft_ours-asymmetric-${ASYMMETRIC_SCALE_TAG}-KL1e-4-beta1.0-fixshift}"
...
echo "Launching H200 8-GPU asymmetric mass-shift experiment with c=${ASYMMETRIC_MASS_SHIFT_SCALE}"

torchrun --standalone --nnodes=1 --nproc_per_node="${NPROC_PER_NODE}" scripts/train_nft_sd3_ours_asymmetric_scaling.py \
