            ...
            for i, train_sample_batch in tqdm(
                ...
                for j_idx, j_timestep_orig_idx in tqdm(
                    ...
                    with torch.no_grad():
                        ...
                        target_velocity = old_prediction.detach().float() + correction_coefficient_expanded * velocity_discrepancy

                    target_velocity_loss = ((forward_prediction - target_velocity) ** 2 * trajectory_alpha).mean(
                        dim=tuple(range(1, x0.ndim))
                    )
