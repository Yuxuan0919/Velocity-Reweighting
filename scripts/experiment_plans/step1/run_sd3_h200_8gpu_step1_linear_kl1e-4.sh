...
export SAVE_DIR="${SAVE_DIR:-${REPO_DIR}/outputs/step1_linear/sd35_${TASK}_h200_8gpu_linear_kl1e-4}"
export RUN_NAME="${RUN_NAME:-sd35_${TASK}_h200_8gpu_linear_kl1e-4}"
...
echo "Step 1 linear (KL coefficient 1e-4): task=${TASK}, GPUs=8, per-device batch=${PER_DEVICE_BATCH}, rollout batches=${NUM_BATCHES_PER_ROUND}, accumulation=${GRADIENT_ACCUMULATION_STEPS}, variance gate=${AWR_VARIANCE_GATE}"

torchrun --standalone --nnodes=1 --nproc_per_node=8 "${TRAIN_SCRIPT}" \
