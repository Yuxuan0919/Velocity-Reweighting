...
# Run two fresh local trajectories sequentially and require all TensorBoard
# scalar events to match in step, order, and serialized float32 bits.
...
docker run --rm --network=none \
  ...
  --volume "${VALIDATION_ROOT}:${VALIDATION_ROOT}:ro" \
  --workdir "${REPO_DIR}" \
  --entrypoint python \
  "${IMAGE}" \
  scripts/compare_tensorboard_all_scalars.py \
  "${VALIDATION_ROOT}/run-a" \
  "${VALIDATION_ROOT}/run-b" \
  --expected-reward-steps "${NUM_STEPS}"
