...
TRAIN_SCRIPT="scripts/train_nft_sd3_ours-4.singleloss-alpha-selection-record.py"
...
LOGDIR="${LOGDIR:-${REPO_DIR}/logs/weight_exps/fix_tie_group}"
SAVE_DIR="${SAVE_DIR:-${REPO_DIR}/outputs/weight_exps/fix_tie_group/sd35_geneval_h200_8gpu_nft_ours-4selection-${EXPERIMENT_TAG}-KL1e-4-beta${EXPERIMENT_BETA}-fulltime-test}"
RUN_NAME="${RUN_NAME:-sd35_geneval_h200_8gpu_nft_ours-4selection-${EXPERIMENT_TAG}-KL1e-4-beta${EXPERIMENT_BETA}-fulltime-test}"
...
echo "Launching H200 tie-aware ${EXPERIMENT_TAG}: 1x${NPROC_PER_NODE} GPUs, per-device batch=${PER_DEVICE_BATCH}, accumulation=${GRADIENT_ACCUMULATION_STEPS}, effective batch=${EFFECTIVE_BATCH}, scheme=${MASS_SHIFT_SCHEME}, rho=${MASS_SHIFT_RHO}, beta=${EXPERIMENT_BETA}"
