...
SOFTMAX_TEMPERATURE_MIN="${SOFTMAX_TEMPERATURE_MIN:-0.01}"
SOFTMAX_TEMPERATURE_MAX="${SOFTMAX_TEMPERATURE_MAX:-0.1}"
...
SAVE_DIR="${REPO_DIR}/outputs/nft_sd3_geneval_nft_ours-weight-d-asymmetric-softmax-t${SOFTMAX_TEMPERATURE_MIN}-${SOFTMAX_TEMPERATURE_MAX}-KL1e-4"
RUN_NAME="sd35_geneval_h200_8gpu_nft_ours-weight-d-asymmetric-softmax-t${SOFTMAX_TEMPERATURE_MIN}-${SOFTMAX_TEMPERATURE_MAX}-KL1e-4"
...
torchrun --standalone --nnodes=1 --nproc_per_node="${NPROC_PER_NODE}" scripts/weight_exps/train_nft_sd3_ours_weight_d_asymmetric_softmax.py \
  --config=config/nft.py:sd3_geneval \
  --softmax_temperature_min="${SOFTMAX_TEMPERATURE_MIN}" \
  --softmax_temperature_max="${SOFTMAX_TEMPERATURE_MAX}" \
