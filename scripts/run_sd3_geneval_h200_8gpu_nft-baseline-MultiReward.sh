...
SAVE_DIR="${SAVE_DIR:-${REPO_DIR}/outputs/nft_sd3_multi_reward_nft_baseline}"
RUN_NAME="${RUN_NAME:-sd35_multi_reward_h200_8gpu_nft_baseline}"
...
if [[ "${CHECK_ONLY:-0}" == "1" ]]; then
  echo "Baseline multi-reward preflight passed (PickScore + HPSv2 + CLIPScore)."
  exit 0
fi

"${PYTHON_BIN}" -m torch.distributed.run --standalone --nnodes=1 --nproc_per_node="${NPROC_PER_NODE}" scripts/train_nft_sd3_origin.py \
