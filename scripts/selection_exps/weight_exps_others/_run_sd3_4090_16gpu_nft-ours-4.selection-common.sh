...
EXPERIMENT_TASK="${EXPERIMENT_TASK:?EXPERIMENT_TASK must be ocr or geneval}"
case "${EXPERIMENT_TASK}" in
  ocr) CONFIG_NAME="sd3_ocr" ;;
  geneval) CONFIG_NAME="sd3_geneval" ;;
  *) echo "EXPERIMENT_TASK must be ocr or geneval" >&2; exit 2 ;;
esac
...
LOGDIR="${LOGDIR:-${REPO_DIR}/logs/weight_exps/weight_exps_others}"
...
case "${NNODES}" in
  ''|*[!0-9]*) echo "NNODES must be a positive integer" >&2; exit 2 ;;
esac
if ((NNODES < 1)); then
...
RUN_NAME="${RUN_NAME:-sd35_${EXPERIMENT_TASK}_4090_${WORLD_SIZE}gpu_nft_ours-4selection-soft-${EXPERIMENT_TAG}-KL1e-4-beta${EXPERIMENT_BETA}-fulltime}"
SAVE_DIR="${SAVE_DIR:-${REPO_DIR}/outputs/weight_exps/weight_exps_others/${RUN_NAME}}"
...
echo "Launching ${EXPERIMENT_TASK} ${EXPERIMENT_TAG} on node ${NODE_RANK}/${NNODES}: ${NNODES}x${NPROC_PER_NODE}=${WORLD_SIZE} GPUs, per-device batch=${PER_DEVICE_BATCH}, accumulation=${GRADIENT_ACCUMULATION_STEPS}, effective batch=${EFFECTIVE_BATCH}, scheme=${MASS_SHIFT_SCHEME}, rho=${MASS_SHIFT_RHO}, beta=${EXPERIMENT_BETA}"
