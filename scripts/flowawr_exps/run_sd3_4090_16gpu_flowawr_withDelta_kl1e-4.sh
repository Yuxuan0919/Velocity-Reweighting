...
TRAIN_SCRIPT="scripts/train_nft_sd3_ours-singleloss-AWR-withDelta.py"
...
SAVE_DIR="${SAVE_DIR:-${REPO_DIR}/outputs/flowawr/sd35_${TASK}_4090_${WORLD_SIZE}gpu_flowawr_withDelta_kl1e-4}"
RUN_NAME="${RUN_NAME:-sd35_${TASK}_4090_${WORLD_SIZE}gpu_flowawr_withDelta_kl1e-4}"
...
echo "FlowAWR (KL coefficient 1e-4): task=${TASK}, node=${NODE_RANK}/${NNODES}, GPUs=${WORLD_SIZE}, per-device batch=${PER_DEVICE_BATCH}, rollout batches=${NUM_BATCHES_PER_ROUND}, accumulation=${GRADIENT_ACCUMULATION_STEPS}, variance gate=${AWR_VARIANCE_GATE}"
