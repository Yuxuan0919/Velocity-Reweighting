...
TRAIN_SCRIPT="scripts/flowawr_exps/train_nft_sd3_ours-singleloss-AWR.py"

# Match the H200 launcher: PickScore by default, with variance gating for
# OCR/GenEval unless AWR_VARIANCE_GATE is explicitly set.
TASK="${TASK:-pickscore}"
...
NNODES="${NNODES:-${SENSECORE_PYTORCH_NNODES:-2}}"
NPROC_PER_NODE="${NPROC_PER_NODE:-${SENSECORE_ACCELERATE_DEVICE_COUNT:-8}}"
NODE_RANK="${NODE_RANK:-${SENSECORE_PYTORCH_NODE_RANK:-${RANK:-${SLURM_NODEID:-}}}}"
case "${NPROC_PER_NODE}" in
  6) DEFAULT_CUDA_VISIBLE_DEVICES="0,1,2,3,4,5" ;;
  8) DEFAULT_CUDA_VISIBLE_DEVICES="0,1,2,3,4,5,6,7" ;;
...
NUM_BATCHES_PER_ROUND=$((1152 / GLOBAL_MICRO_BATCH))
...
if [[ "${REWARD_MAPPING}" == linear ]]; then
  ...
  SAVE_DIR="${SAVE_DIR:-${REPO_DIR}/outputs/step1_linear/sd35_${TASK}_4090_${WORLD_SIZE}gpu_linear_kl1e-4}"
  RUN_NAME="${RUN_NAME:-sd35_${TASK}_4090_${WORLD_SIZE}gpu_linear_kl1e-4}"
else
  LOGDIR="${LOGDIR:-${REPO_DIR}/logs/flowawr}"
  SAVE_DIR="${SAVE_DIR:-${REPO_DIR}/outputs/flowawr/sd35_${TASK}_4090_${WORLD_SIZE}gpu_flowawr_kl1e-4}"
  RUN_NAME="${RUN_NAME:-sd35_${TASK}_4090_${WORLD_SIZE}gpu_flowawr_kl1e-4}"
fi
...
echo "FlowAWR (KL coefficient 1e-4): task=${TASK}, mapping=${REWARD_MAPPING}, node=${NODE_RANK}/${NNODES}, GPUs=${WORLD_SIZE}, per-device batch=${PER_DEVICE_BATCH}, rollout batches=${NUM_BATCHES_PER_ROUND}, accumulation=${GRADIENT_ACCUMULATION_STEPS}, variance gate=${AWR_VARIANCE_GATE}"
