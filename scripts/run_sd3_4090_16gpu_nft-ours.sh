...
if ((NNODES == 1)); then
  ...
  TORCHRUN_DISTRIBUTED_ARGS=(--nnodes="${NNODES}")
fi

echo "Launching ${NNODES}x${NPROC_PER_NODE}=${WORLD_SIZE} GPUs: per-device batch=${PER_DEVICE_BATCH}, accumulation=${GRADIENT_ACCUMULATION_STEPS}, effective batch=${EFFECTIVE_BATCH}"
