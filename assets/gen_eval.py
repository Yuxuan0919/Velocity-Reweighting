...
def load_geneval(DEVICE):
    ...
    def color_classification(image, bboxes, classname):
        ...
        dataset = ImageCrops(image, bboxes)
        if len(dataset) == 0:
            return []
        
        # 直接构建整个 batch（假设 dataset[i][0] 已经是张量）
        images = torch.stack([dataset[i][0] for i in range(len(dataset))]).to(DEVICE)
        
        with torch.no_grad():
            # 添加 autocast，与 zsc.run_classification 保持一致
            with torch.autocast(str(DEVICE), enabled=True):
                image_features = clip_model.encode_image(images)
                image_features = F.normalize(image_features, dim=-1)
                logits = 100. * image_features @ clf
            
            # 转为 float32 后再 argmax，与第一段 pred.float() 行为一致
            pred = logits.float().argmax(dim=1)
            return [COLORS[index.item()] for index in pred]
