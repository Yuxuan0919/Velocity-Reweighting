...
\subsection{Tie-Aware Boundary Allocation}

\paragraph{Tie-group issue.}
...
\paragraph{Tie-aware correction.}
The implementation in
\texttt{scripts/train\_nft\_sd3\_ours-4.singleloss-alpha-selection-tie-aware.py}
first groups samples with exactly equal $|\Delta_i|$.  Starting from the
...
required mass is shared equally across the group.  Weaker groups receive zero.
After the existing division by $\rho$, equal signals therefore have equal
weights and the total mass matches the original mass up to floating-point
roundoff.  Scheme B applies this procedure to both sides, whereas Scheme C
keeps all negative samples and applies it only to the positive side.  At
$\rho=1$, both schemes still give $w_i-1=\Delta_i$.  Only the boundary group
can receive a fractional value; this is necessary to treat tied samples
equally while matching the target mass exactly.  Numerically close but unequal
values are not treated as a tie.
