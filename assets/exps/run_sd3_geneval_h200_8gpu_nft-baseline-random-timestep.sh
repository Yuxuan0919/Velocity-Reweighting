...
SAVE_DIR="${REPO_DIR}/outputs/nft_sd3_geneval_nft_baseline-adapt-KL1e-4-random-timesteps-uni1k-1"
RUN_NAME="sd35_geneval_h200_8gpu_nft_baseline-adapt-KL1e-4-random-timesteps-uni1k-1"
...
torchrun --standalone --nnodes=1 --nproc_per_node="${NPROC_PER_NODE}" scripts/train_nft_sd3_origin-random-timesteps.py \
  ...
  --config.beta=1 \
  --config.train.timestep_sampling_num_steps=1000 \
